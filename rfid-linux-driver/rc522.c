...
static char rc522_loop_work(uchar opnd)
{
    ...
    char status;
	    ...
	    return 0;	
            ...
            if(status!=MI_OK)
            {
                printk("card authorize err");
                return -EFAULT;
            }
            status=PcdWrite(KuaiN,&NewKey[0]);
            if(status!=MI_OK)
            {
                printk("change password err");
                return -EFAULT;
            } else
                printk("set password success");
        } else if (opnd == WRITE_CARD) {//Ð´¿¨
            status=PcdAuthState(PICC_AUTHENT1A,KuaiN,PassWd,MLastSelectedSnr);
            if(status!=MI_OK)
            {
                printk("write authrioze err\n");
                return -EFAULT;               
            }
            ...
            if(status!=MI_OK)
            {
     		...
     		return -EFAULT;
                ...
                printk("write data to block %d sucess\n", KuaiN);
            }
        }
        PcdHalt();	
    return 0;
...
{
	if (KuaiN == 0) {
	printk("block[0] is reserveed, can't write\n");
	return 0;
	}
	if (KuaiN < 0 || KuaiN > 63) {
    ...
    switch(cmd) {
        case CHANGE_PASSWD:
            ...
            if (copy_from_user(PassWd, (char *)arg, sizeof(PassWd))) {
                printk("%s:change pass word err", __func__);
                return -EFAULT;
            }
            break;
        case CHANGE_BLOCK:
            if (arg < 0 || arg > 63) {
                printk("block number err %lu", arg);
                return -EFAULT;
            }
            KuaiN = (int)arg;
            break;
	...
	case GET_ID:
	    ...
	    if(!rc522_loop_work(operationcard)){
	  	  if (copy_to_user((char *)arg, MLastSelectedSnr,4)) {
              		  printk("%s, [line %d] copy to user err.\n", __FILE__, __LINE__);
              		  return -EFAULT;
         	  }
	    }
	    else
